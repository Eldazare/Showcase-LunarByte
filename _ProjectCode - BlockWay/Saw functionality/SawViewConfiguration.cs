	...
	, LevelModel, PlayerModel, ScoreModel, SaveModel, TimerService, SocialService, SaveService>
{
	...
	viewModel.Bind<SawViewModel, float>(superSawScale =>viewModel.SuperSawScaleMultiplier = superSawScale)
		.ToProperty(sawModel, m => m.SuperSawScaleMultiplier, nameof(SawModel.SuperSawScaleMultiplier));
	...
	view.Bind<SawView, float>(view.OnSuperSawScaleChange)
		.ToProperty(viewModel, vm => vm.SuperSawScaleMultiplier,nameof(SawViewModel.SuperSawScaleMultiplier));

	view.Bind<SawView, float>(view.OnBaseScaleChanged)
		.ToProperty(viewModel, vm => vm.SawScaleBase, nameof(SawViewModel.SawScaleBase));
	...
	viewModel.Bind<SawViewModel, float>(sawRotationSpeed =>viewModel.SawRotationSpeed = sawRotationSpeed)
		.ToProperty(sawModel, m => m.SawRotationSpeed, nameof(SawModel.SawRotationSpeed));

	viewModel.Bind<SawViewModel, bool>(superSawActive => viewModel.SuperSawActive = superSawActive)
	...
	viewModel.GatherSuperHayEvent.AddListener(x => playerModel.GatherSuperHay(
					x, 
					sawModel.SuperHayContainerMax,
					() =>{socialService.UnlockAchievement(AchievementIDConstants.FeverID);})
	);

	viewModel.StartDecreasingSuperHayEvent.AddListener(() => timerService.AddConstantTimer(
					sawModel.SuperSawDuration,
					x => playerModel.SetSuperHay(x,sawModel.SuperHayContainerMax /sawModel.SuperSawDuration),
