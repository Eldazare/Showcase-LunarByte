...
public class LevelViewConfigurationWall : ViewConfiguration<ILevelViewModelWall, LevelViewModelWall, LevelViewWall, LevelModelWall, PlayerModel, InputModel, ILevelDatasContainer>
{
            ...
            ILevelDatasContainer levelDataContainer)
    {
        ...
        viewModel.Bind<LevelViewModelWall, ILevelDataWall>(levelData => viewModel.LevelData = levelData).ToProperty(levelModel, m => m.LevelData, nameof(LevelModelWall.LevelData));
        view.Bind<LevelViewWall, ILevelDataWall>(view.LevelDataChanged).ToProperty(viewModel, vm => vm.LevelData, nameof(LevelViewModelWall.LevelData));

        viewModel.Bind<LevelViewModelWall, LevelCommand>(command => viewModel.Command = command).ToProperty(levelModel, m => m.Command, nameof(LevelModelWall.Command));
        view.Bind<LevelViewWall, LevelCommand>(view.ReceiveLevelCommand).ToProperty(viewModel, vm => vm.Command, nameof(LevelViewModelWall.Command));


        viewModel
	        .Bind<LevelViewModelWall, int>(endLevelKeys => viewModel.EndLevelKeys = endLevelKeys)
